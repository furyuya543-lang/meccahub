...
import { ThumbsUp, Map, Tag, BarChart2 } from "lucide-react";
...
export default function HideCard({ hide, rank, featured }: HideCardProps) {
  const diffClass = DIFFICULTY_COLORS[hide.difficulty] ?? "text-gray-400";

  return (
    <Link
      href={`/hide/${hide.id}`}
      className={clsx(
        "group block bg-[#131320] border rounded-xl overflow-hidden hover:border-green-500/40 transition-all hover:shadow-lg hover:shadow-green-900/10",
        featured
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <span className={clsx("text-xs px-2 py-0.5 rounded border", diffClass)}>
            {hide.difficulty}
          </span>
